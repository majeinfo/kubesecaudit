...
func NewRedundantOverrideResult(containerName string, overrideReason, overrideLabel string) *audit.AuditResult {
	return &audit.AuditResult{
		Name:     audit.RedundantAuditorOverride,
		Severity: audit.Warn,
...
func ApplyOverride(auditResult *audit.AuditResult, containerName string, resource k8stypes.Resource, overrideLabel string) *audit.AuditResult {
	...
	auditResult.Severity = audit.Info
	...
	if overrideReason != "" && strings.ToLower(overrideReason) != "true" {
		if auditResult.Metadata == nil {
			auditResult.Metadata = make(audit.Metadata)
		}
