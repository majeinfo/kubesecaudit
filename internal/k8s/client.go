...
func GetAllResources(clientset kubernetes.Interface, options ClientOptions) []k8stypes.Resource {
	...
	resources = append(resources, GetReplicaSets(clientset, options)...)
...
// Get ReplicaSets gets all ReplicaSet resources from the cluster
func GetReplicaSets(clientset kubernetes.Interface, options ClientOptions) []k8stypes.Resource {
	rsClient := clientset.AppsV1().ReplicaSets(options.Namespace)
	rsList, err := rsClient.List(context.Background(), k8stypes.ListOptionsV1{})
	...
	rss := make([]k8stypes.Resource, 0, len(rsList.Items))
	for _, rs := range rsList.Items {
		// For some reason the kubernetes SDK doesn't populate the type meta so we populate it manually
		rs.TypeMeta = k8stypes.NewReplicaSet().TypeMeta
		rss = append(rss, (&rs).DeepCopyObject())
	}
