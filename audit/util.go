...
func auditResources(resources []KubeResource, auditables []Auditable) ([]*AuditResult) {
	var results []*AuditResult

	for _, auditable := range auditables {
		auditResults, err := auditable.Audit(unwrapResources(resources))
		if err != nil {
			continue
		}
		results = append(results, auditResults...)
	}
